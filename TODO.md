...
fix so that it doesnt rebuild the entire thing at once
...
dont load all animation frames up front, but load the first few and then when we are ticking animations we have a record of whats cached which can been looked up

## pretty editor startup

some sort of rendererd thing on the screen would be nice, like telling us that its loading fonts which tend to take a while the more we have, or animation frames etc
